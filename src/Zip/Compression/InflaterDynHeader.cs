...
using System.Linq;
using AotZipLib.SharpZipLib.Zip.Compression.Streams;

namespace AotZipLib.SharpZipLib.Zip.Compression
{
	internal class InflaterDynHeader
	{
		...
		private IEnumerable<bool> CreateStateMachine()
		{
			...
			litLenTree = new InflaterHuffmanTree(GetSegment(new ArraySegment<byte>(codeLengths, 0, litLenCodeCount)));
			distTree = new InflaterHuffmanTree(GetSegment(new ArraySegment<byte>(codeLengths, litLenCodeCount, distanceCodeCount)));

			yield return true;
		}

        public static List<T> GetSegment<T>(ArraySegment<T> SegmentArray)
        {
            List<T> _list = new List<T>();

            for (int i = SegmentArray.Offset; i < (SegmentArray.Offset + SegmentArray.Count); i++)
            {
                _list.Add(SegmentArray.Array[i]);
